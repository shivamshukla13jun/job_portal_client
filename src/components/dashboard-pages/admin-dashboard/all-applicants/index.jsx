...
const index = () => {
  ...
  const { data: jobNames, isLoading } = useQuery({
    queryKey: ['jobNames'],
    queryFn: async () => {
      let res = (await get("admin/jobs/names")).data.data;
  ...
  const { data: currentJob, isLoading: jobLoader } = useQuery({
    queryKey: [`job${job}`,'admin/applications', job],
    queryFn: async () => {
      let res = (await get(`admin/applicantions?job=${job}`)).data;
      return res;
    },
  });

  if (isLoading || jobLoader) return <div>Loading...</div>
const title=jobNames.find(({_id})=>_id==job)?.job?.title
  return (
